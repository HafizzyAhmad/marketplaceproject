...
import { ReactComponent as PersonOutlinedIcon } from '../assets/svg/personOutlineIcon.svg';

const Navbar = () => {
  const navigate = useNavigate();
  const location = useLocation();

// check condition active tab
      ...
      <nav className="navbarNav">
        <ul className="navbarListItems">
          ...
          <li className="navbarListItem" onClick={() => navigate('/profile')}>
            <PersonOutlinedIcon fill={pathMatchRoute('/profile') ? '#2c2c2c' : '#8f8f8f'} width='36px' height='36px'/>
